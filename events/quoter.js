    ...
    async execute (message) {
        ...
        while ((quoteMatch = quoteRegex.exec(message.content))) {
            ...
            if (message.guildId === quoteMatch[1]) {
                ...
                const quoteGuild = message.client.guilds.cache.get(
                    quoteMatch[1]
                );
                if (
            ...
            await quoteChannel.messages?.fetch(quoteMatch[3]).then(
                async (quote) => {
                    ...
                    if (
                        ...
                        quoteAuthor =
                            quote.member.nickname + ' • ' + quoteAuthor;
                    }
                    ...
                    const embed = new MessageEmbed()
                        .setAuthor(
                            quoteAuthor,
                            (quote.member || quote.author).displayAvatarURL(
                                imageOptions
                            )
                        )
                        ...
                        .setFooter(
                            quotedBy,
                            message.member.displayAvatarURL(imageOptions)
                        )
                    ...
                    if (quote.attachments.size) { embed.setImage(quote.attachments.first().url); } else if (!quote.content && quote.embeds.length) {
                        ...
                        while (
                            i < 8 &&
                            quote.embeds[i] &&
                            embeds.reduce((acc, val) => acc + val.length, 0) +
                                embed2.length +
                                quote.embeds[i].length <=
                                5500
                        ) {
